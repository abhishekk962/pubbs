...
def schedule(traveltimeDN, departuretimeDN, departuretimeUP, traveltimeUP, pur, terminalarrivalDN,
    ...
    #defining path
    # path = r'timetable\timetable1\LIFO\vehicleschedule'
    vehicleschedule = []
        ...
        for j in range(0, 2): #j==1 for terminal1 and j==2 for terminal2
            ...
            if or_terminal.loc[i, 'Dep/Arrival'] == 'Departure':
                ...
                if len(temp_pool.index) == 0:  # and or_terminal1.loc[:i,'priority' ].max()==0:
                    ...
                    pool.drop(pool[pool['bus_name'] == temp_pool.loc[0, 'bus_name']].index, inplace=True)
                    pool.reset_index(drop=True, inplace=True)
                    or_terminal.loc[i, 'Pool'] = len(temp_pool.index)
                ...
                # sorting pool based on the minimum time for next departure IN DESCENTING ORDER
                pool.sort_values(by=['Ready_to_depart'], ascending=False, inplace=True)
                pool.reset_index(drop=True, inplace=True)
                temp_pool = pool[pool['Ready_to_depart'] <= or_terminal.loc[i, 'Time of Arrival']]
                or_terminal.loc[i, 'Pool'] = len(temp_pool.index)
    ...
    if pur == 'GA':
        ...
        for i in range(0, len(bus_name)):
            ...
            # df3.to_csv(r'timetable\timetable1\LIFO\vehicleschedule\{}.csv'.format(name))
            df3.title = name
...
def main():
    ...
    #Exporting files
    timetable.to_csv(r'C:\Users\abhis\OneDrive\Desktop\Scripts\busproject\Function files 10122022\LIFO\Time_Table_LIFO.csv', index=False)
    veh_schedule.to_csv(r'C:\Users\abhis\OneDrive\Desktop\Scripts\busproject\Function files 10122022\LIFO\vehicle_schedule_LIFO.csv')
    crew.to_csv(r'C:\Users\abhis\OneDrive\Desktop\Scripts\busproject\Function files 10122022\LIFO\crew_scheduling_LIFO.csv', index=False)
    bus_details.to_csv(r'C:\Users\abhis\OneDrive\Desktop\Scripts\busproject\Function files 10122022\LIFO\Bus_utility_details_LIFO.csv', index=False)
    fig.savefig(r'C:\Users\abhis\OneDrive\Desktop\Scripts\busproject\Function files 10122022\LIFO\vehicle_schedule_visual_LIFO.pdf', dpi=300)
    reuse_buses.to_csv(r'C:\Users\abhis\OneDrive\Desktop\Scripts\busproject\Function files 10122022\LIFO\Reuse_LIFO.csv', index=False)
    fleet.to_csv(r'C:\Users\abhis\OneDrive\Desktop\Scripts\busproject\Function files 10122022\LIFO\fleet_shift_details_LIFO.csv', index=False)
    depot_tt.to_csv(r'C:\Users\abhis\OneDrive\Desktop\Scripts\busproject\Function files 10122022\LIFO\depot_shift_details_LIFO.csv', index=False)
    print('end')
